...
func init() {
	commands.RegisterCommand(requests.C2S_EDITOR_STORE_OBJECT,
		commands.CommandConfig{
			Name:        "StoreObject",
			Identifier:  responses.S2C_EDITOR_STORE_OBJECT,
			Description: "Store an item in the inventory",
			Args:        "{objX} {objY} {objID}",
			MinArgs:     4,
			MaxArgs:     4,
		},
...
func StoreObjectValidator(req *requests.Request, c *client.Client, gm *managers.GameManager, cm commands.CommandConfig) (string, commands.ErrorCodes) {
	...
	if obj == nil || obj.IsDoor() {
		return "No object found at the pos!", commands.ERROR_EDITOR_STORE_FIELD_EMPTY
	}
	...
	if obj.IsStove() || obj.IsCounter() {
		if obj.GetDishID() != -1 {
			return "Cant store object, because theres a dish on top!", commands.ERROR_EDITOR_STORE_OBJ_DISH_ON_TOP_THIS
		}
		...
		if stovesCount == 1 || countersCount == 1 {
			return "Cant store object, because the cafe needs 1 stove and 1 counter!", commands.ERROR_EDITOR_WATCH_OUT
		}
	}
	...
	if obj.IsFridge() || obj.IsDoor() {
		...
		if obj.IsFridge() {
			...
			if 50*(fridgesCount-1) <= c.Location.Cafe().GetFridgeCapacity() {
				return "Cant store the fridge, because the player got too many ingredietns!", commands.ERROR_EDITOR_WATCH_OUT
			}
		}

		if c.Location.Cafe().GetFridgeFreeSpace() < 50*fridgesCount || doorCount == 1 {
			return "Cant store object, because the cafe needs 1 fridge and 1 door!", commands.ERROR_EDITOR_WATCH_OUT
		}
	}
...
func StoreObjectDBSaver(c *client.Client) error {
