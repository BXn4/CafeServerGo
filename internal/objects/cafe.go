...
/*
 def to_response(self, *params: str) -> List[str]:
        return [
            *params, str(self.user_id), str(self.player_id), str(self.player_start[0]),
            str(self.player_start[1]), self.owner_name, str(self.rating), str(self.luxury),
            str(self.expansion_id), str(self.size), str(self.size), str(self.background),
            self.build_tiles(), self.build_objects()
        ]
